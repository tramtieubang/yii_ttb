...
class ImportExcelController extends Controller
{
    ...
    public function actionSaveImport()
    {
        ...
        $count = 0;
        foreach ($rows as $r) {
            $model = new \app\modules\customers\models\CustomersForm();
            $model->name = $r['name'] ?? $r['B'] ?? null;
            $model->email = $r['email'] ?? $r['C'] ?? null;
            $model->phone = $r['phone'] ?? $r['D'] ?? null;
            $model->address = $r['address'] ?? $r['E'] ?? null;
            $model->note = $r['note'] ?? $r['F'] ?? null;
